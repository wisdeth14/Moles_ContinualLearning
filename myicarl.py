...
class myICaRL(SupervisedTemplate):
    """iCaRL Strategy.
    ...
    def __init__(
        ...
        icarl = _myICaRLPlugin(memory_size, buffer_transform, fixed_memory)
...
class _myICaRLPlugin(SupervisedPlugin):
    """
    iCaRL Plugin.
    ...
    def before_training_exp(self, strategy: "SupervisedTemplate", **kwargs):
        ...
        #prob easiest thing to do is don't extend in case of moles???
        #just don't like the idea of code where we "let it know" there is an attack
        # print(benchmark.classes_order[previous_seen_classes: previous_seen_classes + nb_cl])
        # if benchmark.classes_order[previous_seen_classes: previous_seen_classes + nb_cl] not in self.observed_classes:
        #     self.observed_classes.extend(
        #         benchmark.classes_order[
        #         previous_seen_classes: previous_seen_classes + nb_cl
        #         ]
        #     )
        for n in benchmark.classes_order[previous_seen_classes: previous_seen_classes + nb_cl]:
            if n not in self.observed_classes:
                self.observed_classes.append(n)
